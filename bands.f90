...
SUBROUTINE punch_band (filband, spin_component, lsigma, no_overlap)
  ...
  IF ( ionode ) THEN
     ...
     OPEN (unit = hogehoge, file = 'reorderednumber', status = 'unknown',form = &
          'formatted')   !Harada
     REWIND (iunpun)
     DO ipol=1,4
        IF (lsigma(ipol)) THEN
  ...
  ALLOCATE ( reordered_band(nbnd,nkstot)  )  !Harada
  DO ik=1,nkstot
     !
  ...
  IF ( ionode ) THEN
     ...
     DO ibnd=1,nbnd
        ...
        DO ik=nks1tot,nks2tot
           reordered_band(ibnd,ik) = jbnd   !Harada
     ...
     DO ik=nks1tot,nks2tot
        ...
        WRITE (hogehoge, '(10x,3f10.6)', advance='no') xk(1,ik),xk(2,ik),xk(3,ik)    !Harada
        WRITE (hogehoge, '(100I5)') (reordered_band(ibnd,ik), ibnd = 1, nbnd)  !Harada
        DO ipol=1,4
           IF (lsigma(ipol)) THEN
